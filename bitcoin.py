...
def similarity_num(a,b):
    ma=np.mean(a)
    mb=np.mean(b)
    return np.dot((a-ma),(b-mb))
...
def computeDelta(wt, X, Xi):
    ...
    for i in xrange(0,len(Xi.index)):
        ...
        Xirow=Xi.iloc[i]
        b=Xirow[:-1]
        num=similarity_num(a,b)
        den=np.std(a)*np.std(b)*len(a)
        sim_row_i=num/den
        sim_vec=np.append(sim_vec,sim_row_i)

    exp_term=np.exp(wt*sim_vec)    
    yi=Xi.iloc[:,-1]
    delta=np.dot(yi,exp_term)/np.sum(exp_term)
    # YOUR CODE GOES HERE
    return delta
    pass
